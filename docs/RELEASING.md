...
4. Tag the merged commit and push it: `git tag -a vX.Y.Z -m "minfetch vX.Y.Z" && git push origin vX.Y.Z`.
...
6. Update `Formula/minfetch.rb` to the tagged source archive and its SHA-256, then push that formula change to `main`.
7. Verify the public install with `brew update && brew upgrade matheuseabra/minfetch/minfetch` and `brew test matheuseabra/minfetch/minfetch`.
